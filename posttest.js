...
(function(){
  ...
  const IMG = { /* ... (UNCHANGED) ... */
    ...
    mixing:   ['img/mixing_01.png',   'img/mixing_02.png'], cracking: ['img/cracking_01.png', 'img/cracking_02.png'],
    pouring:  ['img/pouring_01.png',  'img/pouring_02.png'], flipping: ['img/flipping_01.png', 'img/flipping_02.png'],
  ...
  function createResilientAudioLoader(candidates){
    ...
    function loadNext(onReady, onFail){
      ...
      const src = asset(candidates[idx++]); audio = new Audio(); audio.preload='auto';
      const clean = () => { if (audio) { audio.removeEventListener('canplaythrough', ok); audio.removeEventListener('error', bad); } };
      const ok  = () => { clean(); onReady?.(audio, src); };
      const bad = () => { clean(); loadNext(onReady, onFail); };
      audio.addEventListener('canplaythrough', ok,  { once:true }); audio.addEventListener('error', bad, { once:true });
      audio.src = src;
    }
    return { loadNext };
  }
  ...
  window.__START_POSTTEST = function(pid, isDelayed) {
    ...
    jsPsych = T('initJsPsych')({
      ...
      on_trial_finish: (data) => { if (data.cleanup_key) { cleanupManager.cleanup(data.cleanup_key); } },
  ...
  function buildTimeline(isDelayed) {
    ...
    if (!have('jsPsychHtmlButtonResponse')) { console.error('Required plugin jsPsychHtmlButtonResponse not found'); return tl; }
    ...
    foley_stimuli.forEach((stim, idx) => {
      ...
      if (idx > 0) {
        if (have('jsPsychHtmlKeyboardResponse')) {
          ...
          tasks.push({ type: T('jsPsychHtmlButtonResponse'), stimulus: `<div style="text-align:center;padding:20px;">Ready for the next sound?</div>`, choices: ['Continue / 続行'] });
        }
      }
      ...
      tasks.push({
        ...
        stimulus: `
          <div style="text-align:center;">
            <div style="padding:20px;"><button id="play-sound-${idx}" class="jspsych-btn">▶️ Play Sound</button><div id="status-${idx}" style="font-size:13px;color:#666;margin-top:8px;">Loading…</div></div>
            <p>What does this sound represent?</p><p style="color:#666;margin-top:4px;">この音は何を表していますか？</p>
          </div>`,
        choices: stim.options,
        button_html: stim.options.map((_, i) => `<button class="jspsych-btn answer-btn" id="answer-btn-${idx}-${i}">%s</button>`),
        data: { task: 'foley_recognition', audio_key: stim.audio, correct_answer: stim.correct, pid: currentPID, condition: testCondition, trial_number: idx + 1, cleanup_key: cleanupKey },
        on_load: function() {
  ...
  function buildPictureNamingTask() {
    ...
    tasks.push({
      type: T('jsPsychHtmlButtonResponse'), stimulus: `<div style="max-width:640px;margin:0 auto;text-align:center"><h3>Picture Description / 絵の説明</h3><p>Describe the picture in English with as much detail as you can — <b>objects, actions, sounds, smells</b>.</p></div>`,
      choices: ['Start / 開始']
    });
    tasks.push({ type: T('jsPsychHtmlAudioResponse'), stimulus: `<div style="text-align:center;"><h4>Microphone check</h4><p>Say "test" for 2 seconds.</p></div>`, recording_duration: 2000, show_done_button: true, allow_playback: true, accept_button_text: 'OK', data: { task: 'mic_check' } });

    const naming_timeline = {
      timeline: [{
        type: T('jsPsychHtmlButtonResponse'),
        stimulus: () => {
          const stim = jsPsych.timelineVariable('stim'); const img = imageSrcFor(stim.target);
        ...
        data: () => ({ task: 'picture_naming_prepare', target: jsPsych.timelineVariable('stim').target, category: jsPsych.timelineVariable('stim').category, trial_index: jsPsych.timelineVariable('idx') })
      }, {
        ...
        data: () => {
          const stim = jsPsych.timelineVariable('stim'); const idx = jsPsych.timelineVariable('idx');
          return { task: 'picture_naming_audio', target: stim.target, category: stim.category, pid: currentPID, phase: 'post', trial_index: idx, cleanup_key: `blob_${currentPID}_${idx}` };
        },
        on_finish: (d) => {
          const tgt = (d.target || 'unknown').toLowerCase(); const idx = d.trial_index || 'x'; d.audio_filename = `post_${currentPID}_${tgt}_${idx}.wav`;
          try {
            const rec = d.response && (d.response instanceof Blob ? d.response : d.response.recording);
            if (rec instanceof Blob) { d.audio_blob_url = URL.createObjectURL(rec); cleanupManager.register(d.cleanup_key, () => { if (d.audio_blob_url) { URL.revokeObjectURL(d.audio_blob_url); } }); }
          } catch(e) { console.error('[posttest] Error processing audio blob:', e); }
        }
      }],
      timeline_variables: picture_naming_stimuli.map((s, i) => ({ stim: s, idx: i + 1 })),
      randomize_order: true
    };
    tasks.push(naming_timeline); return tasks;
  }

  function buildTransferTask() {
    const intro = { type: T('jsPsychHtmlButtonResponse'), stimulus: `<div style="text-align:center;"><h3>Recognition Test / 認識テスト</h3><p>Did this word appear in the VR training?</p></div>`, choices: ['Begin / 開始'] };
    const trial = {
      timeline: [{
        type: T('jsPsychHtmlButtonResponse'), stimulus: jsPsych.timelineVariable('word_display'), choices: ['YES - I saw this', 'NO - I did not see this'], data: jsPsych.timelineVariable('trial_data'),
        on_finish: (d) => {
          const yes = (d.response === 0); d.response_label = yes ? 'yes' : 'no'; d.correct = (yes === d.correct_answer);
          d.signal_type = d.correct_answer ? (yes ? 'hit' : 'miss') : (yes ? 'false_alarm' : 'correct_rejection');
        }
      },
      {
        type: T('jsPsychHtmlButtonResponse'), stimulus: `<div style="text-align:center;"><p>How confident are you?</p><p>どのくらい自信がありますか？</p></div>`, choices: ['1 (Guess)', '2', '3', '4 (Very sure)'], data: { task: 'transfer_confidence' },
      ...
      timeline_variables: transfer_words.map(item => ({
        ...
        trial_data: { task: 'transfer_test', word: item.word, pos: item.pos, iconic: item.iconic, word_type_label: item.type, correct_answer: item.trained, condition: testCondition, pid: currentPID }
      })),
      randomize_order: true
    };
  ...
  function buildPostQuestionnaire() {
    if (!have('jsPsychSurvey')) { return { type: T('jsPsychHtmlButtonResponse'), stimulus: '<p>Survey plugin not available - skipping questionnaire</p>', choices: ['Continue'] }; }
    return {
      type: T('jsPsychSurvey'),
      survey_json: {
        title: 'Post-Training Questionnaire / 訓練後アンケート', showQuestionNumbers: 'off', showCompletedPage: false,
        pages: [{
          elements: [
  ...
  function buildExitOpenQuestion() {
    if (!have('jsPsychSurveyText')) { return { type: T('jsPsychHtmlButtonResponse'), stimulus: '<p>Feedback form unavailable.</p>', choices: ['Finish'] }; }
    return {
      type: T('jsPsychSurveyText'),
      ...
      questions: [{ prompt: 'Your feedback (optional) / フィードバック（任意）', name: 'exit_feedback', rows: 5, required: false }],
